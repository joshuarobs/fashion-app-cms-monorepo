...
interface ColoursTableViewProps {
  layerType?: any;
...
function ColoursTableView({
  layerType,
  selectFabricLayer,
  currentFabricLayerId,
  type = TableType.All_List,
}: ColoursTableViewProps) {
  ...
  const { loading, error, data } = useQuery(Get_Colours_List_BB, {
    variables: {
      limit: 20,
      offset: 0,
    },
  ...
  const newData = _.cloneDeep(data.getColoursListBB);
  newData.forEach((item: any, index: any) => {
    item.key = index;
  });

  // Get the number of all fabric layers
  const numResults = data.getColoursListBB.length;
  ...
  return (
    ...
    >
      ...
      >
        <ColoursTable data={newData} type={type} />
      </Row>
    </Content>
  );
}

export { ColoursTableView };
